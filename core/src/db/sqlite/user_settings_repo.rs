...
use crate::{
    db::user_settings_repo::UserSettingsRepository,
...
impl UserSettingsRepository for SqliteUserSettingsRepository {
    ...
    async fn upsert(&self, user_id: &str, settings: UserNotificationSettings) -> Result<()> {
        ...
        .bind(user_id)
        .bind(bool_to_int(settings.receive_comment_email))
        .bind(bool_to_int(settings.receive_invitation_email))
        .bind(bool_to_int(settings.receive_mention_email))
        .bind(bool_to_int(settings.notify_doc_activity))
        .bind(bool_to_int(settings.notify_permission_change))
        .execute(&self.pool)
        .await?;
