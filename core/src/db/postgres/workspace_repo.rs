...
impl WorkspaceRepository for PostgresWorkspaceRepository {
    async fn update_workspace(&self, params: UpdateWorkspaceParams) -> Result<bool> {
        ...
        if let Some(public) = public {
            ...
            builder.push("public = ");
            builder.push_bind(bool_to_i64(public));
            has_updates = true;
        }
        if let Some(enable_ai) = enable_ai {
            ...
            builder.push_bind(bool_to_i64(enable_ai));
            has_updates = true;
        }
        if let Some(enable_doc_embedding) = enable_doc_embedding {
            ...
            builder.push_bind(bool_to_i64(enable_doc_embedding));
            has_updates = true;
        }
        if let Some(enable_url_preview) = enable_url_preview {
            ...
            builder.push_bind(bool_to_i64(enable_url_preview));
            has_updates = true;
        }
        ...
        if let Some(indexed) = indexed {
            ...
            builder.push_bind(bool_to_i64(indexed));
            has_updates = true;
        }
    ...
    async fn delete_workspace(&self, id: &str) -> Result<bool> {
        let result = pg_query!("DELETE FROM workspaces WHERE id = ?")
            .bind(id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
    ...
    async fn create_workspace(&self, params: CreateWorkspaceParams) -> Result<WorkspaceRecord> {
        ...
        .bind(&owner_id)
        .bind(created_at)
        .bind(bool_to_i64(public))
        .bind(bool_to_i64(enable_ai))
        .bind(bool_to_i64(enable_doc_embedding))
        .bind(bool_to_i64(enable_url_preview))
        .bind(avatar_key.as_ref())
        .bind(bool_to_i64(indexed))
    ...
    async fn list_members_with_users(
        ...
        let rows = pg_query!(
            "SELECT
                 ...
                 u.email_verified_at,
                 u.disabled,
                 (CASE WHEN length(u.password_hash) > 0 THEN 1::BIGINT ELSE 0::BIGINT END) AS has_password,
                 u.created_at
    ...
    async fn delete_member(&self, workspace_id: &str, user_id: &str) -> Result<()> {
        pg_query!("DELETE FROM workspace_members WHERE workspace_id = ? AND user_id = ?")
            .bind(workspace_id)
            .bind(user_id)
            .execute(&self.pool)
            .await?;
    ...
    async fn delete_invite_link(&self, workspace_id: &str) -> Result<bool> {
        let result = pg_query!("DELETE FROM workspace_invite_links WHERE workspace_id = ?")
            .bind(workspace_id)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
