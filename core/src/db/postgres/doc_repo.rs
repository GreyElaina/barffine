...
use crate::{
    ...
    db::postgres::doc_update_log_store::PostgresDocUpdateLogStore,
...
pub struct PostgresDocRepository {
    pool: Pool<Postgres>,
    doc_logs: Arc<PostgresDocUpdateLogStore>,
    doc_snapshots: Arc<DocSnapshotStore>,
}

impl PostgresDocRepository {
    pub fn new(
        pool: Pool<Postgres>,
        doc_logs: Arc<PostgresDocUpdateLogStore>,
        doc_snapshots: Arc<DocSnapshotStore>,
    ...
    fn metadata_from_row(row: PgRow) -> DocumentMetadata {
...
#[async_trait]
impl DocRepository for PostgresDocRepository {
    ...
    async fn unpublish_doc_entry(
        ...
        pg_query!("DELETE FROM doc_public_links WHERE workspace_id = ? AND doc_id = ?")
            .bind(workspace_id)
            .bind(doc_id)
            .execute(&mut *tx)
            .await?;
    ...
    async fn is_doc_public(&self, workspace_id: &str, doc_id: &str) -> Result<bool> {
        let value: Option<i64> =
            pg_query_scalar!("SELECT public FROM documents WHERE workspace_id = ? AND id = ?")
                .bind(workspace_id)
                .bind(doc_id)
                .fetch_optional(&self.pool)
                .await?;
    ...
    async fn find_history_id_before(
        ...
        let history_id: Option<i64> = pg_query_scalar!(
    ...
    async fn list_doc_timestamps(
        ...
        let rows = if let Some(threshold) = after {
            ...
            pg_query!("SELECT id, updated_at FROM documents WHERE workspace_id = ?")
                .bind(workspace_id)
                .fetch_all(&self.pool)
                .await?
        };
