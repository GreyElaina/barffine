...
use crate::{
    ...
    db::{
        blob_repo::{BlobRecord, BlobRepository},
...
impl PostgresBlobRepository {
    ...
    fn build_metadata(row: &PgRow) -> Result<BlobMetadata> {
...
impl BlobRepository for PostgresBlobRepository {
    async fn upsert_blob(
        ...
        let sql = placeholders::convert(table.upsert_sql());
        sqlx::query(sql)
            .bind(identifier)
            .bind(key)
            .bind(content)
    ...
    async fn fetch_blob(&self, scope: &BlobScope, key: &str) -> Result<Option<BlobRecord>> {
        let table = BlobTable::table_for(scope);
        let identifier = scope.identifier();
        let sql = placeholders::convert(table.select_sql());
    ...
    async fn delete_blob(
        ...
        let identifier = scope.identifier();
        if permanently {
            let sql = placeholders::convert(table.hard_delete_sql());
            sqlx::query(sql)
                .bind(identifier)
                .bind(key)
                .execute(&self.pool)
            ...
            let sql = placeholders::convert(table.soft_delete_sql());
            sqlx::query(sql)
                .bind(timestamp)
                .bind(identifier)
                .bind(key)
    ...
    async fn release_deleted(&self, scope: &BlobScope) -> Result<()> {
        let table = BlobTable::table_for(scope);
        let identifier = scope.identifier();
        let sql = placeholders::convert(table.release_deleted_sql());
        sqlx::query(sql)
            .bind(identifier)
            .execute(&self.pool)
            .await?;
    ...
    async fn list_blobs(&self, scope: &BlobScope) -> Result<Vec<ListedBlobRecord>> {
        let table = BlobTable::table_for(scope);
        let identifier = scope.identifier();
        let sql = placeholders::convert(table.list_sql());
    ...
    async fn total_size(&self, scope: &BlobScope) -> Result<i64> {
        let table = BlobTable::table_for(scope);
        let identifier = scope.identifier();
        let sql = placeholders::convert(table.total_size_sql());
        let row = sqlx::query(sql)
            .bind(identifier)
            .fetch_one(&self.pool)
            .await?;
