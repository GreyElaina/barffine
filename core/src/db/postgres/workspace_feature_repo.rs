...
pub struct PostgresWorkspaceFeatureRepository {
    pool: Pool<Postgres>,
}

impl PostgresWorkspaceFeatureRepository {
...
#[async_trait]
impl WorkspaceFeatureRepository for PostgresWorkspaceFeatureRepository {
    async fn list(&self, namespace: &FeatureNamespace) -> Result<Vec<FeatureFlag>> {
        ...
        let rows =
            pg_query!("SELECT key, enabled FROM workspace_feature_flags WHERE workspace_id = ?")
                .bind(workspace_id)
                .fetch_all(&self.pool)
                .await?;
    ...
    async fn remove(&self, workspace_id: &str, key: &str) -> Result<()> {
        pg_query!("DELETE FROM workspace_feature_flags WHERE workspace_id = ? AND key = ?")
            .bind(workspace_id)
            .bind(key)
            .execute(&self.pool)
            .await?;
